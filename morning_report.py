...
async def run_report(
    ...
    if rth_only:
        ...
        # New format: nq-mbo-20260324T1300Z-20260324T2000Z.dbn.zst (start date)
        # Old format: nq-mbo-2026-03-24.dbn.zst
        import re as _re
        _m = _re.search(r'(\d{8})T\d{4}Z', os.path.basename(file_path))
        if _m:
            from datetime import date as _date_cls
            _rth_target_date = _date_cls(
                int(_m.group(1)[:4]),
                int(_m.group(1)[4:6]),
                int(_m.group(1)[6:8]),
            )
    ...
    report_path = file_path.replace(".dbn.zst", "-report.txt")
...
async def run_multi(
    ...
    if len(results) >= 2:
        ...
        last_report_path = file_paths[-1].replace(".dbn.zst", "-report.txt")
